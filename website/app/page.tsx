...
import { buildDesignReference, buildMediaPrompt, detectReferenceCoverage } from "./prompt-builder.js";
...
export default function Home() {
  ...
  const practicePrompt = useMemo(() => {
    const goal = objective || `{{${practiceSelected.fields[0]}}}`;
    const who = audience || `{{${practiceSelected.fields[1]}}}`;
    const output = format || `{{${practiceMode === "casual" ? practiceSelected.fields[4] : practiceSelected.fields[2]}}}`;
    const toneValue = tone || "{{말투}}";
    const lengthValue = length || "{{분량}}";
    if (practiceMode === "goal") return `/goal ${goal}. 검증 방법: ${who}. 허용 범위·중단 조건: ${output}. 반복마다 실제 상태를 확인하고 실패하면 전략을 바꾸세요. 완료 확인 기준과 결과를 대화에 남기세요. /goal은 권한을 자동 승인하지 않으므로 권한 설정은 별도로 확인하세요. 보안·배포·파괴적 변경처럼 위험이 큰 작업은 완료 전 회귀·제약·비밀 노출을 검토하세요.`;
    if (practiceMode === "research") return `조사 질문: ${goal}\n출처 범위: ${who}\n보고서 형식: ${output}\n\n최신성이 필요한 주장은 확인하고, 각 핵심 주장에 출처를 붙이세요. 사실과 추론을 구분해 보고서 형식에 맞춰 반환하세요.`;
    ...
    if (practiceMode === "image") return buildMediaPrompt({ heading: "이미지를 생성하세요.", subjectLabel: "주제·피사체", subject: goal, visualLabel: "시각 스타일·구도", visual: who, formatLabel: "비율·제약", format: output, referencePrompt });
    if (practiceMode === "video") return buildMediaPrompt({ heading: "영상을 생성하세요.", subjectLabel: "장면·행동", subject: goal, visualLabel: "촬영·연출", visual: who, formatLabel: "길이·형식", format: output, referencePrompt, footer: "제공자가 길이·해상도 전용 파라미터를 지원하면 프롬프트와 함께 설정하세요." });
  ...
  }, [practiceMode, practiceSelected, objective, audience, tone, length, format, referencePrompt, designBrief, needsVerification]);
