...
test("server-renders the Prompt Author learning page", async () => {
  ...
  for (const anchor of ["paths", "modes", "practice"]) assert.match(html, new RegExp(`href="#${anchor}"`));
  assert.match(html, /02 \/ MODE SELECTOR/);
  assert.match(html, /03 \/ TRY IT YOURSELF/);
  assert.doesNotMatch(html, /HOW IT WORKS|요청을 받으면 이렇게|정보가 부족하면\?/);
  ...
  assert.match(html, /지금 한 번의 프롬프트가 필요하면 웹에서 바로 생성하고, 특정 프롬프트를 만드는 상황이 반복되면 스킬을 설치하여 사용하세요\./);
...
test("ships the interactive practice tool without starter preview code", async () => {
  ...
  assert.doesNotMatch(page, /id="how"|href="#how"|const flow/);
  for (const anchor of ["paths", "modes", "practice"]) assert.match(page, new RegExp(`href="#${anchor}"`));
  ...
  assert.doesNotMatch(css, /\.flow|\.how/);
